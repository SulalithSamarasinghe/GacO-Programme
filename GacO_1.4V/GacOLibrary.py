...
def start():
    ...
    print('----------------------------------------------------------------------------------------')
    print('')
    print('Welcome!!! GacO 1.4V is a programme which can use to find Gaussian out file results,')
    print('')
    ...
    print('')
    print("Whats new in GacO 1.4V,")
    print('')
    print('     ==> Create final data calculation in xyz.csv file format.')
    print('     ==> Create graphs for the calculated data values.')
...
def help():
    ...
    print('                                            ++++++++++++++++')
    print('')
    print('+ GacO 1.4V is a programme that is mainly focusing on finding the optimized energy values,')
    ...
    print('+ GacO 1.4V creates graphs of Gamma Values VS Wave Length.')
    print('+ Python 3.7 is the base code of the GacO 1.4V programme.')
    print('+ GacO 1.4V consist of 767 Python code lines.')
    print('')
    print('How to enter a Gaussian output file ? ')
    print('  + Gaussian output file and the GacO 1.4V programme must be in the same directory.')
    ...
    def PPGammaMainManu():
        ...
        if Iputg == 1:
            ...
            with open(dataFileName,'w',newline='') as dFile:
                ...
                thewriter.writerow(['Trial_Number','Wave_Length','DC_kerr_Parallel (10**-36 esu)','DC_kerr_Perpendicular (10**-36 esu)','EFISHG_Parallel (10**-36 esu)','EFISHG_Perpendicular (10**-36 esu)'])
                nlen = len(Tnumber)
                for n in range(nlen):
...
#
#
def fileCreator(FileName,type,t1,t2,t3,waveLength,averageName,writeType):
    with open(FileName,writeType,newline='') as dFile:
        thewriter = csv.writer(dFile)
        thewriter.writerow([])
        thewriter.writerow(['Wave_Length','Trial_Number_1 (10**-36 esu)','Trial_Number_2 (10**-36 esu)','Trial_Number_3 (10**-36 esu)',averageName,'Standard_Deviation (10**-36 esu)','Standard_Error_(SE) (10**-36 esu)'])
        nlen = len(waveLength)
        for n in range(nlen):
...
def innerGraphCreator(workbook,worksheet,bold,Graph_Name,gammaType,heading_start,column_start_1,column_end_1,column_start_2,column_end_2,graphStart):
    y_axis_name = gammaType+' (10**-36 esu) Gamma'
    headings = ['Wave Length', y_axis_name]
...
def graphCreator(moleculeName,GF1,GF2,GF3,GF4,GFN1,GFN2,GFN3,GFN4):
    #
    newExcelName = moleculeName+'Graphs.xlsx'
    ...
    innerGraphCreator(workbook,worksheet,bold,GF4,GFN4,'A70','A71','A90','B71','B90','D70')
    workbook.close()
#
#
#
def finalFileCreator():
    ...
    writeType = 'a'
    #
    fileCreator(FileNameDCKP,type1,DCKP1,DCKP2,DCKP3,WaveLengths,averageName,writeType)
    fileCreator(FileNameDCKPP,type2,DCKPP1,DCKPP2,DCKPP3,WaveLengths,averageName,writeType)
    fileCreator(FileNameEFISHGP,type3,EFISHGP1,EFISHGP2,EFISHGP3,WaveLengths,averageName,writeType)
    fileCreator(FileNameEFISHGPP,type4,EFISHGPP1,EFISHGPP2,EFISHGPP3,WaveLengths,averageName,writeType)
